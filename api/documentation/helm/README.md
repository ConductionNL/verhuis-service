# verhuis-service chart

The Helm chart installs verhuis-service and by default the following dependencies using subcharts:
...
```bash
helm repo add verhuis-service https://raw.githubusercontent.com/ConductionNL/verhuis-service/master/api/helm/
helm repo update
```
...
```bash
helm install my-verhuis-service ConductionNL/verhuis-service --version 0.1.0
```
...
| `settings.subdomain` | the subdomain of the installation excluding www. | `verhuis-service` |
| `settings.subpath` | Any sub path to follow the domain, like /api/v1 | `verhuis-service` |
